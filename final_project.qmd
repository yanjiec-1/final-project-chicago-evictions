...
jupyter: final_project_chicago_evictions
format:
  pdf:
    include-in-header:
      text: |
...
\centering
\includegraphics[width=0.8\linewidth]{figures/plot_1_choropleth_eviction_rate_2019.png}
\end{minipage}\hfill
\begin{minipage}{0.47\linewidth}
\centering
...
\begin{minipage}{0.47\linewidth}
\centering
\includegraphics[width=0.8\linewidth]{figures/plot_3_boxplot_eviction_by_minority_quartile.png}
\end{minipage}\hfill
\begin{minipage}{0.47\linewidth}
\centering
...
- Filters designed for subgroup comparison, including income range and minority share range. We use a minority-share range rather than a one-sided cutoff to directly compare low- and high-minority neighborhoods.

- A minimum renter-households filter (default: 200) uses renter households as the denominator of the eviction filing rate and improves stability by reducing the influence of very small denominators that can inflate eviction filing rates.
...
Overall, the dashboard provides more analytic value by including subgroup exploration, variable switching, and extreme-value diagnostics, rather than simply reproducing the static plots.

5. Policy Implications: Because the analysis is descriptive, we do not declare tract characteristics cause eviction filings. However, the hotspot ranking and the dashboard identify where filing rates are most concentrated and support tract targeting and prioritization. Agencies and community organizations could use the identified high rate tracts to help prioritize tenant legal aid outreach, eviction diversion programs, and other resources for housing stability. The tract list also provides a starting point for continuous local monitoring.

6. Limitations or Challenges: This analysis is descriptive and cannot establish causal effects. We focus on 2019 only, which provides a clear snapshot but does not capture time dynamics. Eviction filings are not the same as completed evictions, so filings may overstate displacement. We also rely on tract-level measures (including ACS estimates) that carry sampling error and may not reflect variation within tracts. Finally, while the dashboard provides exploration and diagnostics, conclusions remain constrained by the scope and measurement limits of the underlying data.
